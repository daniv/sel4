...
from pydantic import validate_arguments, Field, FilePath
...
class WebDriverTest(WebDriverBaseTest):
    """ <Class sel4.WebDriverBaseTest> """

    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self._shadow: Optional[ShadowElement] = None
    ...
    @property
    def __shadow_proxy(self) -> ShadowElement:
        if self._shadow is None:
            self._shadow = weakref.proxy(ShadowElement(self))
        return self._shadow

    @staticmethod
    def _fail_if_invalid_shadow_selector_usage(selector: str = Field(..., strict=True, min_length=1)):
    ...
    @staticmethod
    def _is_shadow_selector(selector: str = Field(..., strict=True, min_length=1)):
        WebDriverTest._fail_if_invalid_shadow_selector_usage(selector)
    ...
    def open(self, url: str) -> None:
        ...
        try:
            ...
            self._open_url(self.driver, url, tries=2)
    ...
    def click(
        ...
        if how == By.LINK_TEXT:
            if not self.is_link_text_visible(selector):
                logger.trace("Handle a special case of links hidden in dropdowns")
        ...
        if self._is_shadow_selector(selector):
            self._shadow.shadow_click(selector)
            return
        ...
        def retry_on_stale():
            nonlocal element
            element = element_actions.wait_for_element_interactable(self.driver, how, selector, timeout=timeout)
            try:
                self.__scroll_to_element(element, how, selector)
            except WebDriverException as ex:
                logger.warning(
                    "Exception {} caught while scrolling to element\n{}", ex.__class__.__qualname__, str(ex)
                )
                pass
        ...
        def retry_on_element_not_interactable():
            ...
            try:
                ...
                logger.warning("Exception {} caught while handling anchor\n{}", ex.__class__.__qualname__, str(ex))
                pass
            self.__scroll_to_element(element, selector, by)
            if browser_name == "firefox" or browser_name == "safari":
                handle_safari()
            else:
                element.click()
        ...
        def retry_move_target_or_wd():
            try:
                ...
                try:
                    ...
                    nonlocal element
                    element = element_actions.wait_for_element_interactable(
                        self.driver, how, selector, timeout=timeout
                    )
                    element.click()
        ...
        try:
            ...
            if browser_name == "safari":
                ...
                try:
                    ...
                    logger.warning("Exception {} caught while handling anchor\n{}", e.__class__.__qualname__, str(e))
                    pass
        ...
        latest_ge = latest_window_count > pre_window_count
        switch = settings.SWITCH_TO_NEW_TABS_ON_CLICK
        pre = self.driver.current_url == pre_action_url
        if latest_ge and (switch or pre):
    ...
    def double_click(
        ...
        logger.debug("Find the element one more time in case scrolling hid it")
        element = element_actions.wait_for_element_visible(self.driver, how, selector, timeout=timeout)
        pre_action_url = self.driver.current_url
        try:
            if self.browser == "safari":
                # Jump to the "except" block where the other script should work
                raise WebDriverException("This Exception will be caught.")
    ...
    def update_text(
        ...
        if self._is_shadow_selector(selector):
            self._shadow.shadow_type(selector, text, timeout)
            return
        element = self.wait_for_element_interactable(how, selector, timeout)
        self.__demo_mode_highlight_if_active(how, selector)
        if not self.demo_mode and not self.slow_mode:
            self.__scroll_to_element(element, how, selector)
        try:
        ...
        try:
            ...
            self.wait_for_ready_state_complete()
            time.sleep(0.16)
            element = self.wait_for_element_visible(how, selector, timeout=timeout)
            element.clear()
            if not text.endswith("\n"):
                element.send_keys(text)
            else:
        ...
        if (
            ...
            logger.debug("update_text() is falling back to JavaScript!")
        ...
        if start_page and len(start_page) >= 4:
            if page_utils.is_valid_url(start_page):
                ...
                if page_utils.is_valid_url(new_start_page):
                    ...
                    logger.info('Invalid URL: "%s"!' % start_page)
                    self.open("data:,")
        else:
            self.open("data:,")
    ...
    def is_text_visible(
            self, text: str | None, how: SeleniumBy, selector: str = Field(default="html", strict=True, min_length=1)
    ):
        self.wait_for_ready_state_complete()
        time.sleep(0.01)
        if shadow.is_shadow_selector(selector):
    ...
    @validate_arguments
    def is_link_text_visible(self, link_text: str) -> bool:
        self.wait_for_ready_state_complete()
        time.sleep(0.01)
        return element_actions.is_element_visible(self.driver, link_text, by=By.LINK_TEXT)

    @validate_arguments
    def is_partial_link_text_visible(self, partial_link_text: str) -> bool:
        self.wait_for_ready_state_complete()
        time.sleep(0.01)
        return element_actions.is_element_visible(
            self.driver, partial_link_text, by=By.PARTIAL_LINK_TEXT
        )

    @validate_arguments
    def is_link_text_present(self, link_text: str) -> bool:
    ...
    @validate_arguments
    def is_partial_link_text_present(self, link_text: str) -> bool:
    ...
    def get_attribute(
        ...
        time.sleep(0.01)
        if self._is_shadow_selector(selector):
            return self._shadow._get_shadow_attribute(
                selector, attribute, timeout=timeout
            )
        element_actions.wait_for_element_present(self.driver, how, selector, timeout)
        try:
            attribute_value = element.get_attribute(attribute)
        except (StaleElementReferenceException, ElementNotInteractableException):
            self.wait_for_ready_state_complete()
            time.sleep(0.14)
            element = element_actions.wait_for_element_present(self.driver, how, selector, timeout)
            attribute_value = element.get_attribute(attribute)
    ...
    def set_attribute(
        ...
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.SMALL_TIMEOUT)
        if scroll and self.is_element_visible(how, selector):
            try:
                self.scroll_to(selector, by=by, timeout=timeout)
            except Exception:
                pass
        attribute = re.escape(attribute)
        attribute = shared.escape_quotes_if_needed(attribute)
        value = re.escape(value)
        value = shared.escape_quotes_if_needed(value)
        css_selector = self.convert_to_css_selector(selector, by=by)
        css_selector = re.escape(css_selector)  # Add "\\" to special chars
        css_selector = shared.escape_quotes_if_needed(css_selector)
        script = (
    ...
    def remove_attribute(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1),
            timeout: OptionalInt = None,
        ...
        css_selector = re.escape(css_selector)  # Add "\\" to special chars
        css_selector = shared.escape_quotes_if_needed(css_selector)
        script = f"document.querySelector('{css_selector}').removeAttribute('{attribute_name}');"
        self.execute_script(script)
    ...
    def remove_attributes(
        ...
        script = """var $elements = document.querySelectorAll('%s');
                  var index = 0, length = $elements.length;
                  for(; index < length; index++){
                  $elements[index].removeAttribute('%s');}""" % (css_selector, attribute_name)
        try:
            self.execute_script(script)
        except WebDriverException | JavascriptException as e:
            logger.warning(
                "Exception {} caught while removing attribute from element ->\n{}",
                e.__class__.__qualname__, str(e)
            )
            pass
    ...
    def has_attribute(
        ...
        timeout = self.get_timeout(timeout, constants.SMALL_TIMEOUT)
        element = element_actions.wait_for_element_present(self.driver, how, selector, timeout)
        return element_actions.has_attribute(element, attribute_name)

    def get_property(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1),
            property_name=Field(..., min_length=1),
            timeout: OptionalInt = None
    ) -> Optional[Any]:
        ...
        time.sleep(0.01)
        element = element_actions.wait_for_element_present(self.driver, how, selector, timeout)
        return element.get_property(property).strip()
    ...
    def get_text_content(
        ...
        self.__check_scope__()
        return self.get_property(how, selector, property_name, timeout)
    ...
    def get_property_value(
        ...
        try:
            ...
            raise WebDriverException(
                "Exception: Could not convert {how=how}({selector}) to CSS_SELECTOR!"
            )
        selector = re.escape(selector)
        selector = shared.escape_quotes_if_needed(selector)
    ...
    def get_image_url(
        ...
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.SMALL_TIMEOUT)
        return self.get_attribute(how, selector, "src", timeout)

    def find_elements(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1), limit=0
    ) -> List[WebElement]:
        ...
        self.wait_for_ready_state_complete()
        time.sleep(0.05)
        elements = element_actions.find_elements(self.driver, how, selector)
    ...
    def click_visible_elements(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1),
            limit: int = 0, timeout: OptionalInt = None
    ) -> None:
        ...
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.SMALL_TIMEOUT)
        self.wait_for_element_present(how, selector, timeout=timeout)
        elements = self.find_elements(how, selector)
        ...
        for element in elements:
            if limit and 0 < limit <= click_count:
                return
            try:
    ...
    def click_nth_visible_element(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1),
            number: int = 0, timeout: OptionalInt = None
    ):
        ...
        self.wait_for_ready_state_complete()
        self.wait_for_element_present(how, selector, timeout=timeout)
        elements = self.find_visible_elements(how, selector)
        if len(elements) < number:
            raise WebDriverException(
                f"Not enough matching {selector} elements of type {how.upper()} to "
                f"click number {number}!"
            )
        number = number - 1
        if number < 0:
            number = 0
        ...
        try:
            ...
            if len(elements) < number:
                raise WebDriverException(
                    f"Not enough matching {selector} elements of type {how} to click number {number}!"
                )
            number = number - 1
            if number < 0:
                number = 0
    ...
    def is_selected(
        ...
        """ Same as is_checked() """
        return self.is_checked(how, selector, timeout=timeout)

    def check_if_unchecked(self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1)):
        """ If a checkbox or radio button is not checked, will check it. """
        self.__check_scope__()
        if not self.is_checked(how, selector):
            if self.is_element_visible(how, selector):
                self.click(how, selector)
            else:
                selector = self.convert_to_css_selector(how, selector)
                self.js_click(By.CSS_SELECTOR, selector)

    def select_if_unselected(self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1)):
        """ Same as check_if_unchecked() """
        self.check_if_unchecked(how, selector)

    def uncheck_if_checked(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1)
    ):
        """ If a checkbox is checked, will uncheck it. """
        self.__check_scope__()
        if self.is_checked(how, selector):
            if self.is_element_visible(how, selector):
                self.click(how, selector)
            else:
    ...
    def unselect_if_selected(self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1)):
        """ Same as uncheck_if_checked() """
        self.uncheck_if_checked(how, selector)

    def is_element_in_an_iframe(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1)
    ):
        """Returns True if the selector's element is located in an iframe.
        Otherwise returns False."""
        self.__check_scope__()
        if self.is_element_present(how, selector):
            return False
        soup = self.get_beautiful_soup()
    ...
    def switch_to_frame_of_element(
            self, how: SeleniumBy, selector: str = Field(..., strict=True, min_length=1)
    ) -> None:
    ...
    def hover_on_element(
        ...
        if page_utils.is_xpath_selector(selector):
            ...
            by = By.CSS_SELECTOR
        self.wait_for_element_visible(how, selector, timeout=settings.SMALL_TIMEOUT)
        self.__demo_mode_highlight_if_active(how, selector)
        self.scroll_to(how, selector)
    ...
    def install_addon(self, xpi_file: FilePath):
        ...
        self.wait_for_ready_state_complete()
        if self.driver.capabilities.get("browseName") != "firefox":
            raise Exception(
    ...
    def set_value(
        ...
        self.__demo_mode_highlight_if_active(how, selector)
        if scroll and not self.demo_mode and not self.slow_mode:
            self.scroll_to(how, selector, timeout)
        if type(text) is int or type(text) is float:
            text = str(text)
    ...
    def js_type(
        ...
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.LARGE_TIMEOUT)
        self.js_update_text(text, selector, how, timeout)
    ...
    def set_text(
        ...
        if element.tag_name == "input" or element.tag_name == "textarea":
            self.js_update_text(text, how, selector, timeout)
        else:
            self.set_text_content(text, how, selector, timeout)
    ...
    def set_text_content(
        ...
        element = element_actions.wait_for_element_present(self.driver, how, selector, timeout)
        if element.tag_name == "input" or element.tag_name == "textarea":
            self.js_update_text(text, how, selector, timeout=timeout)
            return
        css_selector = self.convert_to_css_selector(how, selector)
        if scroll:
            self.__demo_mode_highlight_if_active(how, selector)
    ...
    def jquery_update_text(
        ...
        selector = shared.escape_quotes_if_needed(selector)
        text = re.escape(text)
        text = shared.escape_quotes_if_needed(text)
    ...
    def get_element(
        ...
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.SMALL_TIMEOUT)
        return self.wait_for_element_present(how, selector, timeout=timeout)
    ...
    @validate_arguments
    def wait_for_partial_link_text(self, partial_link_text: str, timeout: OptionalInt = None):
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.LARGE_TIMEOUT)
        return self.wait_for_element_visible(partial_link_text, by=By.PARTIAL_LINK_TEXT, timeout=timeout)

    @validate_arguments
    def find_partial_link_text(self, partial_link_text: str, timeout: OptionalInt = None):
    ...
    def wait_for_element_absent(
        ...
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.LARGE_TIMEOUT)
        return element_actions.wait_for_element_absent(self.driver, selector, by, timeout)
    ...
    def wait_for_element_not_visible(
        ...
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.LARGE_TIMEOUT)
        return element_actions.wait_for_element_not_visible(self.driver, selector, by, timeout)
    ...
    def wait_for_text_not_visible(
            ...
            text: str,
            how: SeleniumBy,
            selector: str = Field(default="html", strict=True, min_length=1),
            timeout: OptionalInt = constants.LARGE_TIMEOUT
    ):
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.LARGE_TIMEOUT)
        return element_actions.wait_for_text_not_visible(self.driver, how, selector, timeout)

    def wait_for_attribute_not_present(
            self,
            attribute_name: str,
            attribute_value: str | bool | float | int | None,
            how: SeleniumBy,
            selector: str = Field(default="html", strict=True, min_length=1),
            timeout: OptionalInt = None
    ):
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.LARGE_TIMEOUT)
        return element_actions.wait_for_attribute_not_present(
            self.driver, attribute_name, attribute_value, how, selector, timeout
        )
    
    def wait_for_and_accept_alert(self, timeout: OptionalInt = None):
        self.__check_scope__()
        timeout = self.get_timeout(timeout, constants.LARGE_TIMEOUT)
        return element_actions.wait_for_and_accept_alert(self.driver, timeout)
    ...
    def __scroll_to_element(self, element, selector=None, by=By.CSS_SELECTOR):
        ...
        if not success and selector:
            self.wait_for_ready_state_complete()
            element = element_actions.wait_for_element_visible(
    ...
    def setup(self):
        ...
        if self.is_pytest:
            ...
            if not self.config.getoption("disable_ws", False):
                self.config.option.enable_ws = True
    ...
    def teardown(self):
        ...
        """
        if self._called_teardown:
            logger.info("This test already called teardown()")
            return
        self._called_teardown = True
        ...
        if self.config.getoption("headless") or self.config.getoption("xvfb"):
        ...
        if self.config.getoption("with_s3_logging and has_exception", False):
