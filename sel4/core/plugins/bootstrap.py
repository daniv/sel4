...
class EnvironmentModel(pydantic.BaseModel):
    environment: constants.Environments = pydantic.Field(default=None, description="The working environment")
...
def pytest_load_initial_conftests(early_config: "Config", parser: "Parser", args: List[str]) -> None:
    ...
    patched.trace("Setting environment 'PROJECT_ROOT' to {rootdir}", rootdir=str(early_config.rootdir))
    ...
    if settings_module is None:
        ...
        if expression is None:
            ...
            settings_file = constants.Environments.settings().get(constants.Environments(env_value))
            os.environ.setdefault('SEL4_SETTINGS_MODULE', settings_file)
            EnvironmentModel(environment=env_value)
            parser.addini("environment", type="string", default=env_value, help="The sel4 working environment")
