...
class LazySettings(LazyObject):
    """
    A lazy proxy for either global Kiru settings or a custom settings object.
    The user can manually configure settings prior to using them. Otherwise,
    it uses the settings module pointed to by KIRU_SETTINGS_MODULE.
    """
        ...
        if not settings_module:
            ...
            exc = ImproperlyConfigured(
                f"""
                    ...
                    "You must either define the environment variable {ENVIRONMENT_VARIABLE} "
                    "or call settings.configure() before accessing settings."
                """
            )
            getattr(exc, "_add_error_label")("EnvironmentError")
            raise exc
    ...
    def __getattr__(self, name):
        ...
        if name in {'MEDIA_URL', 'STATIC_URL'} and val is not None:
            ...
            getattr(exc, "_add_error_label")("SecretsFileName")
            raise
