...
class PytestCache:
    ...
    def delete_cache_older_than_x_days(self, cache_path: pathlib.Path, days: int):
        ...
        for prefix in (self._CACHE_PREFIX_DIRS, self._CACHE_PREFIX_VALUES):
            setup_logger.debug(
                "Scanning cache directory '{prefix}' for files older than: {days} days", prefix=prefix, days=abs(days)
            )
            curr_dir = cache_path / prefix
            from sel4.utils.fileutils import iter_find_files
            files = list(
                iter_find_files(curr_dir, '*', ignored=['stepwise', 'nodeids', 'lastfailed'])
            )
            for file in files:
